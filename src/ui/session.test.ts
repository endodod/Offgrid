...
/** A Session over a small hand-built map - Session itself has no DOM dependency, only Hud does. Missions no
 *  longer pre-select a unit (the player's first click should be a deliberate one), so tests that need a
 *  selection call selectFirstPlayer() explicitly. */
...
const selectFirstPlayer = (session: Session) => session.select(session.state.units.find((u) => u.team === 'player')!.id);
