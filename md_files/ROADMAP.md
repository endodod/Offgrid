...
**Resolved**
- Profile is per mission/squad (`MapDef.enemyProfile` / `GameOptions.enemyProfile`), not per unit or per spawn - the simplest scope that still lets a mission characterize its whole enemy force, matching how weather/time-of-day are already mission-level rather than per-tile. Per-spawn granularity is a straightforward later extension (the `Spawn` tuple would need a 4th element) if a mission ever wants a mixed squad.
