...
- **Campaign levels are hand-authored, one file each, in `src/data/maps/`** - four Act 1 story missions and five supply-run layouts, each designed around a single objective type rather than sharing a layout. `src/data/maps.test.ts` holds the contract every one of them must satisfy (rectangular, known tile characters, spawns and interactables on open tiles, one of each class per player squad, and everything reachable from the squad's spawn with doors treated as openable). `npm run sim -- --map <id>` balance-checks any of them; each map file records its own numbers in its header comment. See [STORY.md](STORY.md) for the per-level design sheet.
